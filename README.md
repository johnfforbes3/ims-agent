...
**Current status: Phase 17 complete — voice added as a channel on the existing Teams text path.** CAMs can now reply by typing OR by sending a Teams voice message — both routes feed the same `ChatInterviewSession` → `InterviewAgent` → `cycle_runner` pipeline. ATLAS replies are sent as both text and a TTS audio attachment so CAMs can read or listen. The standalone `/voice/test` web tester drives the production `InterviewAgent` (no divergent code paths). **Zero changes to production interview behavior, cycle_runner, dashboard endpoints, cam_inputs shape, or any downstream analysis.** Env-flag-gated: `TEAMS_VOICE_IN=true` (default on), `TEAMS_VOICE_OUT=true` (default on), `VOICE_AGENT_V2_TESTER=true` (opt-in for the browser tester). All Phase 16 productionization features (real-data endpoints, cycle heartbeat, LLM circuit breaker, immutable audit log) are unchanged. **83/83 voice unit tests passing + 60/60 voice eval scenarios + 11/11 conversation flow scenarios + 708/708 existing default suite.** Rollback path: unset the env flags to return to Phase 16 behavior; tag `pre-phase17-voice-upgrade-2026-05-17` for hard revert. See `docs/PHASE-17-INTEGRATION-PLAN.md` for the architecture and `docs/PHASE-17-DAY1-REPORT.md` for the iteration history.
