...
**Current count (Phase 15):** 770/770 non-legacy unit tests passing as of 2026-05-08 + 63 new Phase 15 tests + 407 legacy dashboard tests (Phase 12/12.1/14) intentionally skipped via `@pytest.mark.legacy` (4 Whisper integration tests skipped — `openai-whisper` not installed; 1 pre-existing test-order flake in `test_cycle_not_active_before_trigger`, passes in isolation).

**Phase 15 dashboard rebuild (2026-05-08):** complete replacement of the Phase 12/14 server-rendered Jinja2 + vanilla-JS dashboard with a **React 18 + Babel Standalone** single-page app dropped in from a user-supplied design zip. All third-party deps vendored locally at `/static/vendor/` (React 18.3.1 production minified, Babel 7.29.0 standalone, 11 IBM Plex Latin woff2 fonts via rewritten Google-Fonts CSS) — zero CDN dependencies. Source code lives at `/static/atlas/`: 7 files from the zip (`styles.css`, `data.js`, `components.jsx`, `charts.jsx`, `IMSStats.jsx`, `PMPortal.jsx`, `AgentControls.jsx`, `app.jsx`) plus 1 NEW file `api.js` that fetches `/api/state`, `/api/evm/history`, `/api/health/history`, `/api/diff/latest`, `/api/changes`, `/api/baseline-drift` and overrides 11 mock `window.*` globals with live data before React mounts. `app.jsx` awaits `window.__IMS_HYDRATE` before `ReactDOM.createRoot(...)`. Live Interview Listen-In on the Agent Controls tab opens a real EventSource on `/api/interview-stream` (with backfill from `/api/interview-recent`); falls back to the scripted demo loop when no live interview is active. FORCE CYCLE button calls `/api/trigger?force=true`. **Core agent code unchanged** — `server.py`, `cycle_runner.py`, `interview_orchestrator.py`, `voice/interview_agent.py`, `llm_interface.py`, `file_handler.py`, `critical_path.py`, `sra_runner.py`, `cam_directory.py`, and every other backend module are untouched. The Phase 14 `base.html` is preserved as `base.legacy.html`. Phase 12/12.1/14 tests are marked `@pytest.mark.legacy` and skipped by default (enable via `pytest -m legacy` or `IMS_LEGACY_DASHBOARD=1`). 63 new tests in `tests/test_phase15_dashboard_rebuild.py` cover React shell, vendored assets, JSX module exports, data + api layer, agent API regression, and the legacy rollback path. Rollback tag: `pre-dashboard-zip-rebuild-2026-05-08`.

**Phase 14 modern polish (2026-05-08):** Visual modernization layered on top of Phase 12.1 — purely additive, no structural change. Glassmorphism on every card via `backdrop-filter: blur(14px) saturate(140%)` plus a 1px white inset highlight. Animated body gradient (3 radial color washes with a 40s diagonal pan, fixed under the content). Conic-gradient progress rings (`.progress-ring` utility class driving a CSS conic gradient from `--pct`) on the HIGH Risk Milestones tile and CAMs Responded tile. Animated KPI counters via `animateNumber()` in `dashboard-core.js` with `easeOutQuart` easing — every `.kpi-value[data-target]` element counts up from 0 over 900ms. View Transitions API hook on `_activateTab` (`document.startViewTransition()` with `view-transition-name` pseudo-elements for crossfade + slide). Hover micro-interactions: cards lift, chips lift, buttons press, logo rotates, gradient underline grows from center on tab buttons. Aurora strip (animated gradient line) under the header. Breathing animation on the cycle-in-progress card. Chart.js global defaults tuned (900ms `easeOutQuart`, polished tooltips, thicker lines, rounded bars). All animations honor `prefers-reduced-motion: reduce`. Test class `tests/test_phase14_modern_polish.py` (47 tests across 8 classes). Rollback tag: `pre-modern-polish-2026-05-08`.
