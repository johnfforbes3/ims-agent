...
**Current count:** 1115/1115 tests passing as of 2026-05-08 (4 Whisper integration tests skipped — `openai-whisper` not installed). Zero open low/medium TDs.
...
**Phase 13 visual redesign (2026-05-08):** Major UI overhaul. The dashboard is now a 3-column **app-shell** (`<div class="app-shell">` with CSS grid `sidebar | topbar | main+rightbar`). Left sidebar (`.app-sidebar`) holds the brand block, primary tab nav (Workspace section with the 3 buttons), Quick Actions (Executive Briefing + Demo Mode), and a footer segmented sun/moon theme switch. Top bar (`.app-topbar`) hosts a pill-shaped search input, refresh-countdown chip, notification bell (red dot when validation holds present), Trigger Cycle CTA, and a profile avatar. Right widget panel (`.app-rightbar`) shows Cycle Status, Active Interviews live pills, Recent Cycles timeline, and Top Risks preview. Light theme is the new default (`<html data-theme="light">`); dark theme preserved via design-token overrides under `[data-theme="dark"]`. Design tokens (CSS custom properties) live under `:root` for surfaces, text, borders, accents (warm coral `#ff5a36`), health palette, typography scale, spacing, radius, shadows. Inter web font loaded via Google Fonts. New hero trend chart on the metrics tab (canvas `#metrics-trend-hero`) plots SPI/CPI/BEI overlaid across 24 cycles via `_renderHeroTrend()` in `metrics-tab.js`. Test class `tests/test_phase13_visual_redesign.py` (47 tests across 8 classes) covers app-shell, sidebar, topbar, rightbar, design tokens, theme defaults, hero chart, and Phase 12 regression guard. Rollback tag: `pre-visual-redesign-2026-05-08`.
